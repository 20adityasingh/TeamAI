...
public class ProjectTemplateServiceImpl implements ProjectTemplateService {
    ...
    public void initializeProjectFromTemplate(Long projectId) {
        log.info("Attempting to initialize project {} from template. Target Bucket: {}, Template Bucket: {}, Template Name: {}", 
                projectId, TARGET_BUCKET, TEMPLATE_BUCKET, TEMPLATE_NAME);
        ...
        try {
            ...
            log.info("Target bucket {} exists check: {}", TARGET_BUCKET, targetBucketExists);
            ...
            log.info("Listing objects in {} with prefix {}", TEMPLATE_BUCKET, TEMPLATE_NAME + "/");
            ...
            for (Result<Item> result : results) {
                ...
                log.debug("Found object in template: {}", sourceKey);

                // Skip directory entries (they end with / or have isDir flag)
                if (item.isDir() || sourceKey.endsWith("/")) {
                    log.debug("Skipping directory entry: {}", sourceKey);
                    continue;
                }
                ...
                try {
                    ...
                    log.error("Failed to copy individual file {}: {}", sourceKey, e.getMessage());
                }
            }
            ...
            if (!filesToSave.isEmpty()) {
                ...
                log.warn("No files were copied for project {}. Template prefix used: {}", projectId, TEMPLATE_NAME + "/");
            }

        } catch (Exception e) {
            log.error("Critical failure during project template initialization for project {}: {}", projectId, e.getMessage(), e);
